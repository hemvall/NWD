...
function setLabel(weightKg: number, reps: number): string {
  return weightKg > 0 ? `${weightKg} kg × ${reps}` : `${reps} reps (PdC)`
}
...
export default function LiftsPanel({ entries, profile, onAdd, onDelete }: Props) {
  ...
  function submit(e: React.FormEvent) {
    ...
    onAdd({ lift, weightKg: weight ? +weight : 0, reps: Math.round(+reps), date })
    setWeight('')
    setReps('')
  }
  ...
  return (
    <div className="glass-card p-5">
      ...
      {/* Add form */}
      <form onSubmit={submit} className="grid grid-cols-2 sm:grid-cols-5 gap-2 mb-2">
      ...
      <p className="text-[10px] font-mono text-cyan-100/25 mb-4">Laissez le poids vide pour une série au poids du corps (tractions, dips…).</p>
          ...
          return (
            <div key={k} className="glass-inner rounded-lg p-3">
              ...
              <p className="text-base font-mono font-bold neon-text-cyan mt-0.5">
                {best ? setLabel(best.weightKg, best.reps) : '--'}
              </p>
              {progress !== null && (
                <>
          ...
          {[...entries].reverse().slice(0, 8).map(e => (
            <div key={e.id} className="flex items-center justify-between glass-inner rounded-md px-3 py-1.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-mono text-cyan-100/60">{setLabel(e.weightKg, e.reps)}</span>
              </div>
